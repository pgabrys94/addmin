...
def privkey_check(priv_path, pub_path):
    ...
    if os.path.exists(priv_path):
            ...
            if os.path.exists(pub_path):
                ...
                if len(imported_pubkey) == 0 or "ssh-rsa" not in imported_pubkey.split():
                    ...
                    if pubkey == imported_pubkey.split()[1]:
                        ...
                        time.sleep(1)
                        print("PUBKEY: ", pubkey)
                        print("IMP PUBKEY: ", imported_pubkey)
                        print("returning pubkey")
                        return pubkey
                    else:
                ...
                try:
                    pubkey = privkey.get_base64()
                    full_pubkey = f"ssh-rsa {pubkey} addmin@{os.uname().nodename}"
                    with open(pub_path, "w") as f:
                        f.write(full_pubkey)
                    print("Pubkey autogenerated from privkey...")
                    time.sleep(1)
                    print("returning full_pubkey")
                    return full_pubkey
        ...
        with open(pub_path, "w") as f:
            full_pubkey = f"ssh-rsa {public_key} addmin@{os.uname().nodename}"
            f.write(full_pubkey)
